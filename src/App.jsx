...
import {
  ...
  Hammer, Wrench, Box, Cog, Menu, Map, User,
  Thermometer, Wind, AlertTriangle, CheckSquare, Square
} from 'lucide-react';

const SurvivalGameUI = () => {
  // --- STATE ---
  const [activeLeftTab, setActiveLeftTab] = useState('tasks'); // 'inventory' | 'tasks'
  const [activeModal, setActiveModal] = useState(null);

  // Falešná herní data pro UI
  ...
  const handleBuildClick = (nodeName) => {
    setActiveModal(`build_${nodeName}`);
  };

  // --- KOMPONENTY ---

  // Horní lišta: Čas, Stavy, Suroviny
  const TopBar = () => (
    <div className="flex items-center justify-between bg-stone-950 border-b-2 border-amber-900/50 p-2 text-amber-100 text-sm shadow-md z-10 font-mono">
      {/* Čas a Den */}
      <div className="flex space-x-4 items-center bg-stone-900 px-3 py-1 rounded border border-stone-700">
        <span className="font-bold text-amber-500">DEN 24</span>
        <span className="text-stone-400">07:18 AM</span>
      </div>

      {/* Stavy (Health, Food, atd.) */}
      ...
      <div className="flex space-x-6">
        <ResourceItem icon={<Cog size={16} />} value={resources.scrap} />
        <ResourceItem icon={<Box size={16} />} value={resources.wood} />
        <ResourceItem icon={<Flame size={16} />} value={resources.coal} />
        <ResourceItem icon={<Wrench size={16} />} value={resources.parts} />
  ...
  const StatBar = ({ icon, value, color }) => (
    <div className="flex items-center space-x-2 bg-stone-900/80 px-2 py-1 rounded border border-stone-800">
      {icon}
  ...
  // Levý panel: Nástroje a Úkoly/Inventář
  const LeftSidebar = () => (
  ...
  // Hlavní plocha herního světa (Střed)
  const GameCanvas = () => (
    <div className="flex-1 relative bg-stone-950 overflow-hidden flex items-center justify-center">
      {/* Pozadí - Mřížka nákresu */}
      <div className="absolute inset-0 opacity-10"
           style={{ backgroundImage: 'linear-gradient(#333 1px, transparent 1px), linear-gradient(90deg, #333 1px, transparent 1px)', backgroundSize: '20px 20px' }}>
      </div>

      {/* Průřez Domu - Kontejner */}
      <div className="relative w-full max-w-4xl aspect-[16/9] border-2 border-stone-800/50 rounded-lg p-8">

        {/* Nápisy místností (pozadí) */}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <div className="relative w-64 h-96 bg-gradient-to-b from-stone-700 to-stone-900 border-4 border-amber-900 rounded-t-full rounded-b-lg shadow-2xl flex flex-col items-center justify-end pb-8 pointer-events-auto cursor-pointer hover:border-amber-500 transition group" onClick={() => setActiveModal('boiler')}>
            {/* Budíky */}
            ...
            <div className="text-center mb-4">
              ...
              <div className="text-green-500 text-xs font-mono font-bold animate-pulse">Stav: V PROVOZU</div>
            </div>

            {/* Dvířka */}
            <div className="w-32 h-24 border-2 border-stone-950 bg-stone-900 rounded-t-lg relative overflow-hidden group-hover:border-amber-600">
              ...
              <div className="absolute bottom-0 w-full h-16 flex justify-evenly items-end pb-2">
                 {[1,2,3,4,5].map(i => (
                    <div key={i} className={`w-3 rounded-t-full bg-gradient-to-t from-yellow-300 via-orange-500 to-red-600 shadow-[0_0_15px_rgba(255,165,0,0.8)] animate-pulse`} style={{ animationDelay: `${i*0.1}s`, height: `${32 + (i%3)*8}px`}}></div>
                 ))}
              </div>
            </div>
          </div>
        ...
        {/* Trubky spojující uzly */}
        <div className="absolute top-[60%] left-[20%] w-[30%] h-4 border-t-4 border-l-4 border-amber-900 rounded-tl-xl opacity-50"></div>
        <div className="absolute top-[65%] right-[20%] w-[30%] h-4 border-t-4 border-r-4 border-amber-900 rounded-tr-xl opacity-50"></div>

        {/* Uzel: Dynamo (Vlevo dole) */}
        <div className="absolute bottom-16 left-16">
          <BuildNode
            title="DYNAMO"
            icon={<Zap size={32} className="text-stone-400 group-hover:text-yellow-400" />}
            onClick={() => handleBuildClick('dynamo')}
            status="BUILD"
          />
        </div>

        {/* Uzel: Pěstírna (Vpravo dole) */}
        <div className="absolute bottom-16 right-16">
          <BuildNode
            title="PĚSTÍRNA"
            icon={<Wind size={32} className="text-stone-400 group-hover:text-green-400" />}
            onClick={() => handleBuildClick('bed')}
            status="BUILD"
          />
        </div>
  ...
  const BuildNode = ({ title, icon, onClick, status }) => (
    <div
      className="group flex flex-col items-center cursor-pointer transform hover:scale-105 transition-transform"
      onClick={onClick}
    >
      <div className="w-24 h-24 bg-stone-900/80 border-2 border-stone-700 group-hover:border-amber-500 rounded relative flex items-center justify-center shadow-lg backdrop-blur-sm">
      ...
      <div className="bg-amber-900/50 text-amber-400 text-xs px-3 py-0.5 rounded-full mt-1 border border-amber-700/50">
        {status}
      </div>
    </div>
  );

  // Spodní lišta: Menu stavby a akce
  const BottomBar = () => (
    <div className="bg-stone-950 border-t-2 border-amber-900/50 p-3 flex items-center justify-between z-10">
      <div className="w-64"></div>

      {/* Menu Stavby (Uprostřed) */}
  ...
  const Modal = () => {
    ...
    let content = { title: 'Informace', body: '...' };
    if (activeModal === 'boiler') {
      content = { title: 'HLAVNÍ KOTEL', body: 'Kotel aktuálně topí. Spotřeba: 2 Uhlí / hodina. Výkon: 71%.' };
    } else if (activeModal === 'build_dynamo') {
      content = { title: 'POSTAVIT: DYNAMO', body: 'Vyžaduje: 50 Šrotu, 10 Součástek. Zajistí stabilní přísun energie pro dílnu.' };
    } else if (activeModal === 'build_bed') {
      content = { title: 'POSTAVIT: PĚSTÍRNA', body: 'Vyžaduje: 30 Dřeva, 15 Šrotu. Umožní pěstovat bylinky a zeleninu.' };
    } else if (activeModal === 'map') {
      content = { title: 'MAPA OKOLÍ', body: 'Připravte skupinu na noční výpravu (Scavenging).' };
    }

    return (
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center font-mono">
        <div className="bg-stone-900 border-2 border-amber-700 p-6 rounded-lg max-w-md w-full shadow-2xl relative">
          <button
            onClick={() => setActiveModal(null)}
            className="absolute top-2 right-2 text-stone-500 hover:text-red-500 font-bold p-1"
          >
            ✕
          </button>
          <h3 className="text-xl font-bold text-amber-500 mb-4 border-b border-stone-700 pb-2">{content.title}</h3>
          <p className="text-stone-300 mb-6">{content.body}</p>
          <div className="flex justify-end space-x-3">
            <button
              className="px-4 py-2 bg-stone-800 border border-stone-600 text-stone-300 hover:bg-stone-700 rounded transition"
              onClick={() => setActiveModal(null)}
            >
              Zrušit
            </button>
            <button
              className="px-4 py-2 bg-amber-800 border border-amber-600 text-amber-100 font-bold hover:bg-amber-700 rounded transition"
              onClick={() => setActiveModal(null)}
            >
              Potvrdit
            </button>
